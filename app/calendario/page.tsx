...
type Agendamento = {
  ...
  tipo: string; notas: string; cliente_id: string | null
  clientes: { nome: string } | null
}
type Cliente = { id: string; nome: string }
type ClientePopup = { id: string | null; nome: string }
...
export default function CalendarioPage() {
  ...
  const [lembreteEnviado, setLembreteEnviado] = useState<string | null>(null)
  ...
  function iniciarEdicao(a: Agendamento) {
    setClientePopup(null); setEditandoId(a.id); setClienteId(a.cliente_id || '')
    setHoraInicio(a.hora_inicio || ''); setHoraFim(a.hora_fim || ''); setTipo(a.tipo || ''); setNotas(a.notas || '')
    setMostrarForm(true)
  }
  ...
  async function guardarAgendamento(e: React.FormEvent) {
    ...
    if (editandoId) {
      await supabase.from('agendamentos').update({ cliente_id: clienteId || null, data: dataStr, hora_inicio: horaInicio || null, hora_fim: horaFim || null, tipo: tipo || null, notas: notas || null }).eq('id', editandoId)
    } else {
      const { data: { user } } = await supabase.auth.getUser()
      await supabase.from('agendamentos').insert({ cliente_id: clienteId || null, fisio_id: user?.id, data: dataStr, hora_inicio: horaInicio || null, hora_fim: horaFim || null, tipo: tipo || null, notas: notas || null })
    }
    cancelarForm(); setLoading(false); carregarAgendamentos()
  }

  async function apagarAgendamento(id: string) {
    if (!confirm(t.delete + '?')) return
    await supabase.from('agendamentos').delete().eq('id', id)
    setClientePopup(null); carregarAgendamentos()
  }
  ...
  function clientesNoDia(dia: number): { cliente: ClientePopup; ags: Agendamento[] }[] {
    ...
    ags.forEach(a => {
      ...
      if (!mapa.has(key)) mapa.set(key, { cliente: { id: a.cliente_id, nome: a.clientes?.nome || t.noClient }, ags: [] })
  ...
  async function enviarLembrete(a: Agendamento) {
    ...
    try {
      const res = await fetch('/api/lembrete', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ clienteNome: clientePopup?.nome || t.noClient, data: a.data, horaInicio: a.hora_inicio, horaFim: a.hora_fim, tipo: a.tipo, notas: a.notas }) })
      if (res.ok) { setLembreteEnviado(a.id); setTimeout(() => setLembreteEnviado(null), 3000) }
      else alert(t.error)
    } catch { alert(t.error) }
  ...
  const agendamentosDoClientePopup = clientePopup ? agendamentosNoDia(diaSelected!).filter(a => (a.cliente_id ?? null) === clientePopup.id) : []
  ...
  return (
    <main style={s.page}>
      <div style={{ maxWidth: '600px', margin: '0 auto' }}>
        <Voltar />
        <h1 style={{ fontSize: '32px', fontWeight: 800, color: '#fff', textTransform: 'uppercase', letterSpacing: '-0.01em', marginBottom: '24px' }}>{t.calendar}</h1>
        ...
        <div style={s.card}>
          ...
          <div style={s.grid}>{diasSemana.map((d, i) => <div key={i} style={s.dayLabel}>{d}</div>)}</div>
          <div style={s.grid}>
            ...
            {Array.from({ length: diasNoMes() }).map((_, i) => {
              ...
              return (
                <button key={dia} onClick={() => { setDiaSelected(dia); cancelarForm(); setClientePopup(null) }}
                  style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', aspectRatio: '1/1', borderRadius: '8px', fontSize: '11px', fontWeight: 700, border: 'none', cursor: 'pointer', background: isSelected ? '#3b82f6' : isHoje ? '#1a1a1a' : 'transparent', color: isSelected ? '#fff' : isHoje ? '#fff' : '#888' }}>
                  <span>{dia}</span>
        ...
        {diaSelected && (
          <div style={s.card}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
              ...
              <button style={s.btnGhost} onClick={() => { cancelarForm(); setMostrarForm(true) }}>+ {t.newSchedule}</button>
            </div>
            ...
            {mostrarForm && (
              <form onSubmit={guardarAgendamento} style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '16px', paddingBottom: '16px', borderBottom: '1px solid #1a1a1a' }}>
                <p style={{ fontSize: '9px', color: editandoId ? '#a855f7' : '#3b82f6', textTransform: 'uppercase', letterSpacing: '0.15em', fontWeight: 700, margin: 0 }}>
                  {editandoId ? '✎ ' + t.edit : '+ ' + t.newSchedule}
                </p>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                  <div>
                    <p style={{ fontSize: '9px', color: '#555', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '6px' }}>{t.time} Início</p>
                    <input type="time" value={horaInicio} onChange={e => setHoraInicio(e.target.value)} style={s.input} />
                  </div>
                  <div>
                    <p style={{ fontSize: '9px', color: '#555', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '6px' }}>{t.time} Fim</p>
                    <input type="time" value={horaFim} onChange={e => setHoraFim(e.target.value)} style={s.input} />
                  </div>
                </div>
                <input value={tipo} onChange={e => setTipo(e.target.value)} placeholder={t.session} style={s.input} />
                <textarea value={notas} onChange={e => setNotas(e.target.value)} placeholder={t.notes} rows={2} style={{ ...s.input, resize: 'none' }} />
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button type="submit" disabled={loading} style={s.btnBlue(!!editandoId)}>{editandoId ? t.update : t.save}</button>
                  <button type="button" onClick={cancelarForm} style={s.btnGhost}>{t.cancel}</button>
                </div>
              </form>
            )}

            {clientesDoDia.length === 0 ? (
              <p style={{ fontSize: '11px', color: '#444', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{t.noResults}</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {clientesDoDia.map(({ cliente, ags }) => (
                  <button key={cliente.id || 'sem-cliente'}
                    onClick={() => setClientePopup(clientePopup?.id === cliente.id ? null : cliente)}
                    style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#0d0d0d', border: '1px solid #1e1e1e', borderRadius: '12px', padding: '14px 16px', cursor: 'pointer', width: '100%', textAlign: 'left' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      <div style={{ width: '3px', height: '32px', background: '#3b82f6', borderRadius: '2px' }} />
                      <div>
                        <p style={{ fontSize: '12px', fontWeight: 700, color: '#fff', textTransform: 'uppercase', letterSpacing: '0.05em', margin: 0 }}>{cliente.nome}</p>
                        <p style={{ fontSize: '10px', color: '#555', textTransform: 'uppercase', letterSpacing: '0.05em', margin: 0, marginTop: '2px' }}>
                          {ags.length} {t.schedules.toLowerCase()}{ags[0]?.hora_inicio ? ` · ${ags[0].hora_inicio.slice(0, 5)}` : ''}
                        </p>
                      </div>
                    </div>
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 50, transform: clientePopup ? 'translateY(0)' : 'translateY(100%)', transition: 'transform 0.3s cubic-bezier(0.32, 0.72, 0, 1)', background: '#111', borderTop: '1px solid #1e1e1e', borderRadius: '24px 24px 0 0', maxHeight: '80vh', overflowY: 'auto' }}>
        ...
        {clientePopup && (
          <div style={{ padding: '20px 24px 40px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
              <div>
                <p style={{ fontSize: '10px', color: '#3b82f6', textTransform: 'uppercase', letterSpacing: '0.15em', fontWeight: 700, marginBottom: '4px' }}>{dataSelectedStr}</p>
                <h2 style={{ fontSize: '20px', fontWeight: 800, color: '#fff', textTransform: 'uppercase', letterSpacing: '-0.02em', margin: 0 }}>{clientePopup.nome}</h2>
              </div>
                ...
                <div key={a.id} style={{ background: '#0d0d0d', border: '1px solid #1e1e1e', borderRadius: '14px', padding: '16px' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginLeft: '12px' }}>
                      ...
                      <button onClick={() => enviarLembrete(a)} disabled={enviandoLembrete === a.id}
