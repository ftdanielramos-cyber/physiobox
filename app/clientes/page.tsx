          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div style={{ background: '#111', border: '1px solid #1a1a1a', borderRadius: '16px', padding: '20px' }}>
              ...
              <BodyChart
                ...
                onChange={async (novasRegioes) => {
                  setRegioes(novasRegioes)
                  if (ficha) {
                    await supabase.from('fichas').update({ regioes_afetadas: novasRegioes }).eq('id', ficha.id)
                  } else {
                    const { data: novaFicha } = await supabase.from('fichas').insert({ cliente_id: id, regioes_afetadas: novasRegioes, historico_medico: '', patologias: '', medicacao: '', observacoes: '' }).select().single()
                    if (novaFicha) setFicha(novaFicha)
                  }
                }}
              />
            </div>
