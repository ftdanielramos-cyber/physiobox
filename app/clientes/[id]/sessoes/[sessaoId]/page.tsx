...
type SetDB = { id: string; numero: number; repeticoes: number; carga: number }
...
export default function SessaoPage() {
  ...
  function ajustarSet(index: number, campo: 'repeticoes' | 'carga', delta: number) {
    setSets(prev => { const novos = [...prev]; novos[index] = { ...novos[index], [campo]: Math.max(0, novos[index][campo] + delta) }; return novos })
  }

  function iniciarHold(index: number, campo: 'repeticoes' | 'carga', delta: number) {
    ajustarSet(index, campo, delta)
    let velocidade = 400
    function passo() { ajustarSet(index, campo, delta); velocidade = Math.max(50, velocidade - 60); holdRef.current = setTimeout(passo, velocidade) }
  ...
  function iniciarEdicao(r: Registo) {
    setEditandoId(r.id); setTipo(r.tipo as 'exercicio' | 'intervencao'); setNomeExercicio(r.nome_exercicio || '')
    setDescricao(r.descricao || ''); setBilateral(r.bilateral ?? true); setLado(r.lado || 'esquerdo'); setNotas(r.notas || '')
    if (r.sets && r.sets.length > 0) setSets([...r.sets].sort((a, b) => a.numero - b.numero).map(s => ({ numero: s.numero, repeticoes: s.repeticoes, carga: s.carga })))
    else setSets([{ numero: 1, repeticoes: 10, carga: 0 }])
    setMostrarForm(true)
  }
  ...
  async function guardarRegisto(e: React.FormEvent) {
    e.preventDefault()
    if (editandoId) {
      await supabase.from('registos').update({ tipo, nome_exercicio: tipo === 'exercicio' ? nomeExercicio : null, descricao: tipo === 'intervencao' ? descricao : null, bilateral: tipo === 'exercicio' ? bilateral : null, lado: tipo === 'exercicio' && !bilateral ? lado : null, notas: notas || null }).eq('id', editandoId)
      if (tipo === 'exercicio') { await supabase.from('sets').delete().eq('registo_id', editandoId); await supabase.from('sets').insert(sets.map(s => ({ registo_id: editandoId, numero: s.numero, repeticoes: s.repeticoes, carga: s.carga }))) }
    } else {
      const { data: registo } = await supabase.from('registos').insert({ sessao_id: sessaoId, tipo, nome_exercicio: tipo === 'exercicio' ? nomeExercicio : null, descricao: tipo === 'intervencao' ? descricao : null, bilateral: tipo === 'exercicio' ? bilateral : null, lado: tipo === 'exercicio' && !bilateral ? lado : null, notas: notas || null, ordem: registos.length }).select().single()
      if (registo && tipo === 'exercicio') await supabase.from('sets').insert(sets.map(s => ({ registo_id: registo.id, numero: s.numero, repeticoes: s.repeticoes, carga: s.carga })))
    }
  ...
  function downloadPDF() {
    ...
    janela.document.write(gerarHTMLReport())
                    ...
                    {r.tipo === 'exercicio' && r.sets && r.sets.length > 0 && (
                      <div style={{ marginTop: '10px', display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                        {[...r.sets].sort((a, b) => a.numero - b.numero).map(set => (
                          <span key={set.id} style={{ fontSize: '10px', color: '#aaa', background: '#0d0d0d', border: '1px solid #1e1e1e', borderRadius: '8px', padding: '4px 8px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{set.repeticoes}×{set.carga}kg</span>
                        ))}
                      </div>
                    )}
          ...
          {etapaPopup === 'opcoes' ? (
            <>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                ...
                <button onClick={() => router.push('/dashboard')} style={{ width: '100%', background: 'transparent', color: '#555', border: '1px solid #1e1e1e', borderRadius: '14px', padding: '14px', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', cursor: 'pointer' }}>
                  Terminar sem Report
                </button>
              </div>
            </>
