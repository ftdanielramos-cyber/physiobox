      ...
      <div className="max-w-[600px] mx-auto">
        ...
        <div className="grid grid-cols-3 gap-2.5 mb-7">
          {[
            ...
            { valor: sessoesMes, label: 'Este Mes' },
          ].map(st => (
        ...
        {proximos.length > 0 && (
          <div className="mb-7">
            <p className="text-[10px] text-white/25 tracking-[0.16em] uppercase mb-2.5">Proximos agendamentos</p>
            <div className="flex flex-col gap-2.5">
              {proximos.map(a => {
                const isHoje = a.data === hojeStr
                return (
                  <a key={a.id} href={a.cliente_id ? '/clientes/' + a.cliente_id : '/calendario'} className="bg-white/[0.03] border border-white/[0.06] rounded-xl px-4 py-3.5 flex items-center gap-3.5 no-underline hover:border-white/[0.12] transition-colors">
                    <div className="w-1 h-10 bg-blue-500/60 rounded-full shrink-0" />
                    <div className="flex-1 min-w-0">
                      <p className="text-[14px] font-medium text-white uppercase tracking-[0.04em]">{a.clientes?.nome || 'Sem Cliente'}</p>
                    ...
                    {isHoje && (
                      <span className="text-[10px] bg-blue-500/15 text-blue-400 rounded-md px-2 py-0.5 tracking-[0.06em] uppercase">Hoje</span>
                    )}
                  </a>
                )
              })}
        ...
        <p className="text-[10px] text-white/25 tracking-[0.16em] uppercase mb-2.5">Navegacao</p>
        <div className="flex flex-col gap-2.5">
