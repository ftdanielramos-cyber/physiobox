...
export default function Exercicios() {
  ...
  const [form, setForm] = useState({
    nome: '', categoria: '', descricao: '', notas: '', youtube_url: ''
  })
  ...
  function abrirNovo() {
    setEditando(null)
    setForm({ nome: '', categoria: '', descricao: '', notas: '', youtube_url: '' })
    setModalOpen(true)
  }
  ...
  return (
    <main style={s.page}>
        ...
        <div style={{ marginBottom: '28px', borderBottom: '1px solid #1a1a1a', paddingBottom: '24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <button onClick={abrirNovo} style={{
              ...
              padding: '10px 16px', color: '#fff', fontSize: '11px',
                  ...
                  <div style={{ padding: '16px', display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
                    ...
                    <div
                      ...
                      style={{
                        ...
                        background: ytId ? `url(https://img.youtube.com/vi/${ytId}/mqdefault.jpg) center/cover` : '#1d1d1d',
                        border: '1px solid #2a2a2a', display: 'flex', alignItems: 'center', justifyContent: 'center',
                        cursor: ytId ? 'pointer' : 'default', overflow: 'hidden', position: 'relative',
                      }}>
                      {ytId && (
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                        ...
                        {ex.categoria && (
                          <span style={{ fontSize: '8px', fontWeight: 700, color: '#3b82f6', background: 'rgba(59,130,246,0.1)', border: '1px solid rgba(59,130,246,0.2)', borderRadius: '20px', padding: '2px 8px', textTransform: 'uppercase', letterSpacing: '0.1em', whiteSpace: 'nowrap' }}>
                            {ex.categoria}
                          </span>
                        )}
                      </div>
        ...
        <div onClick={() => setModalOpen(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 100, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', padding: '0' }}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#111', border: '1px solid #222', borderRadius: '20px 20px 0 0', padding: '28px 20px 40px', width: '100%', maxWidth: '600px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div>
              ...
              <input value={form.nome} onChange={e => setForm(f => ({ ...f, nome: e.target.value }))} placeholder="Ex: Agachamento" style={s.input} />
            </div>
            ...
            <div>
              ...
              <textarea value={form.descricao} onChange={e => setForm(f => ({ ...f, descricao: e.target.value }))}
                placeholder="Descrição do exercício..." rows={3}
            ...
            <div>
              ...
              <textarea value={form.notas} onChange={e => setForm(f => ({ ...f, notas: e.target.value }))}
                placeholder="Notas clínicas, progressões..." rows={2}
                style={{ ...s.input, resize: 'none' as const }} />
            </div>
            ...
            <div style={{ display: 'flex', gap: '10px', marginTop: '4px' }}>
              <button onClick={() => setModalOpen(false)} style={{ flex: 1, padding: '12px', background: 'transparent', border: '1px solid #222', borderRadius: '10px', color: '#555', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', cursor: 'pointer' }}>
                Cancelar
              </button>
              <button onClick={guardar} disabled={saving} style={{ flex: 2, padding: '12px', background: '#3b82f6', border: 'none', borderRadius: '10px', color: '#fff', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', cursor: 'pointer', opacity: saving ? 0.6 : 1 }}>
      ...
      {/* Modal vídeo */}
      {videoAberto && (
        <div onClick={() => setVideoAberto(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.9)', zIndex: 200, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div onClick={e => e.stopPropagation()} style={{ width: '100%', maxWidth: '600px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
              ...
              <button onClick={() => setVideoAberto(null)} style={{ background: 'none', border: 'none', color: '#666', cursor: 'pointer' }}>
                <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>
              </button>
            </div>
            <div style={{ position: 'relative', paddingBottom: '56.25%', height: 0, borderRadius: '12px', overflow: 'hidden' }}>
              <iframe
                src={`https://www.youtube.com/embed/${getYoutubeId(videoAberto.youtube_url || '')}?autoplay=1`}
            ...
            {videoAberto.notas && (
              <p style={{ color: '#3b82f6', fontSize: '12px', marginTop: '12px', lineHeight: 1.5 }}>📝 {videoAberto.notas}</p>
            )}
          </div>
        </div>
      )}
